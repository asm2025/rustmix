...
use quantized_nn::{layer_norm, linear, Linear};
...
#[derive(Debug, Clone)]
struct Embedding {
    wte: quantized_nn::Embedding,
}

impl Embedding {
    fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        let wte = quantized_nn::Embedding::new(cfg.vocab_size, cfg.n_embd, vb.pp("wte"))?;
        Ok(Self { wte })
    }
}
...
impl RotaryEmbedding {
    ...
    fn apply_rotary_emb_qkv(
        ...
        if three != 3 {
            candle_core::bail!("unexpected shape for qkv {:?}", qkv.shape())
        }
...
impl MHA {
    ...
    fn forward(
        ...
        let seqlen_offset = match &cache {
            Some(ParallelBlockCache(Some(ParallelBlockCacheValue { key, .. }))) => key.dim(1)?,
            _ => 0,
        };
...
impl ParallelBlock {
    ...
    fn forward(
        ...
        let attn_outputs = self.mixer.forward(&xs, mask, cache)?;
...
impl PhiCache {
    ...
    pub fn get_tensor_map(&self) -> HashMap<String, Tensor> {
        ...
        for (i, block) in self.blocks.iter().enumerate() {
            if let ParallelBlockCache(Some(ParallelBlockCacheValue { key, value })) = block {
                map.insert(format!("phi.cache.blocks.{}.key", i), key.clone());
                map.insert(format!("phi.cache.blocks.{}.value", i), value.clone());
            }
        }
        map
    }
    ...
    pub fn from_tensor_map(map: HashMap<String, Tensor>) -> Self {
        ...
        for (k, v) in map {
            if let Some(i) = k.strip_prefix("phi.cache.blocks.") {
                let i = i
                    .strip_suffix(".key")
                    .unwrap_or_else(|| i.strip_suffix(".value").unwrap());
                let i = i.parse::<usize>().unwrap_or(0);
                if i >= blocks.len() {
                    blocks.resize(i + 1, ParallelBlockCache(None));
                }
                if k.ends_with(".key") {
                    match blocks.get_mut(i) {
                        Some(ParallelBlockCache(Some(ParallelBlockCacheValue {
                            key,
                            value: _,
                        }))) => {
                            *key = v;
                        ...
                        _ => {
                            blocks[i] = ParallelBlockCache(Some(ParallelBlockCacheValue {
                                key: v.clone(),
                                value: v,
                            }));
                        }
